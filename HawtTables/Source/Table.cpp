...
namespace HawtLib {
	...
	namespace HawtTables {
		...
		void Table::_AddDivider(size_t rowIdx) const{
			...
			Card* dividerCard = new Card{ m_This, std::string((m_BiggestCardLength), '-'), '+' };
		...
		Table& Table::AddCard(const std::string& text, unsigned int rowIdx, char bCap) {
			...
			m_Rows[rowIdx]->Cards.emplace_back(new Card{ this, text, bCap });	// add card to specific row
